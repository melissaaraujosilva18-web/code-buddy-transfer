...
const Auth = () => {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-background/95 p-4">
      <Dialog open={true} onOpenChange={open => !open && navigate("/")}>
        <DialogContent className="max-w-[90vw] sm:max-w-md p-0 bg-[#1a1a1a] border-none overflow-hidden">
          <div className="relative h-[180px] sm:h-[200px] overflow-hidden">
            {activeTab === "login" ? <img src={bannerLogin} alt="Seja Bem-Vindo(a)" className="w-full h-full object-cover" /> : <img src={bannerBonus} alt="Bônus de Boas Vindas" className="w-full h-full object-cover" />}
          </div>
              ...
              <TabsContent value="login" className="mt-0">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                  <div className="relative">
                    ...
                    <Input type="email" name="email" placeholder="Digite o E-mail" required className="pl-11 bg-[#2a2a2a] border-none text-white placeholder:text-gray-500 h-12" />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-primary" />
                    <Input type="password" name="password" placeholder="Digite a senha" required className="pl-11 bg-[#2a2a2a] border-none text-white placeholder:text-gray-500 h-12" />
                  </div>
                  <Button type="submit" className="w-full bg-primary hover:bg-primary/90 text-white h-12 font-semibold text-base shadow-[0_0_20px_hsl(var(--neon-green-glow)/0.3)]" disabled={isLoading}>
                    {isLoading ? "Entrando..." : "Entrar"}
                  </Button>
                  <div className="text-center">
                    <span className="text-white text-sm">Ainda não possui uma conta? </span>
                    <button type="button" onClick={() => setActiveTab("signup")} className="text-primary hover:text-primary/80 font-semibold text-sm transition-colors">
                      Registrar
                    </button>
                  </div>
                </form>
              ...
              <TabsContent value="signup" className="mt-0">
                ...
                <form onSubmit={handleSignup} className="space-y-4">
                  <div className="relative">
                    ...
                    <Input type="text" name="fullName" placeholder="Digite o nome" required className="pl-11 bg-[#2a2a2a] border-none text-white placeholder:text-gray-500 h-12" />
                  </div>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-primary" />
                    <Input type="email" name="email" placeholder="Digite o E-mail" required className="pl-11 bg-[#2a2a2a] border-none text-white placeholder:text-gray-500 h-12" />
                  ...
                  <div className="relative">
                    ...
                    <Input type="text" name="cpf" placeholder="Digite seu CPF" required className="pl-11 bg-[#2a2a2a] border-none text-white placeholder:text-gray-500 h-12" />
                  </div>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-primary" />
                    <Input type="password" name="password" placeholder="Digite a senha" required minLength={6} className="pl-11 bg-[#2a2a2a] border-none text-white placeholder:text-gray-500 h-12" />
                  </div>
                  <div className="flex items-center space-x-2">
                    <Checkbox id="terms" checked={acceptTerms} onCheckedChange={checked => setAcceptTerms(checked as boolean)} className="border-gray-600 data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
                  ...
                  <div className="text-center">
                    ...
                    <button type="button" onClick={() => setActiveTab("login")} className="text-primary hover:text-primary/80 font-semibold text-sm transition-colors">
                      Entrar
                    </button>
                  </div>
                </form>
